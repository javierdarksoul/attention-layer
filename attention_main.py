...
train(model,train_loader,valid_loader, params['train']['epoch'], params['train']['learnrate'], params['train']['usecuda'] ,params['train']['pathsave'] , "metrics_attention.json")


model.load_state_dict(torch.load(params['train']['pathsave'] + "/bestx.pt"))
model= model.cpu()
predictions=[]
labels=[]
...
with open("precision_attention.json", "w") as f:
...
with open("recall_attention.json", "w") as f:
            f.write(json.dumps(kpi_dict))
...
with open("f1_score_attention.json", "w") as f:
            f.write(json.dumps(kpi_dict))